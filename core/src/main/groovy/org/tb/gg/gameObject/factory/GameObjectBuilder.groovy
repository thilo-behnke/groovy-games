...
class GameObjectBuilder<T extends GameObject> implements Builder<GameObject> {
    ...
    T build() {
        if(!gameObject.renderComponent || !gameObject.inputComponent || !gameObject.physicsComponent) {
            throw new IllegalStateException("A game object must have a render, an input and a physics component!")
        }
        return (T) gameObject
    }
}
